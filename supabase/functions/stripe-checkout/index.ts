...
Deno.serve(async (req) => {
  try {
    ...
    if (!tokenPackage.stripe_price_id) {
      console.error('Package missing stripe_price_id:', package_id)
      return new Response(
        JSON.stringify({ error: 'Package not configured for purchase' }),
        {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    console.log(`Creating checkout for package: ${tokenPackage.name} (${tokenPackage.token_count} tokens)`)
