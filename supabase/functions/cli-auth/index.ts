...
const getAuthURL = () => {
  ...
  if (environment === "staging") {
    return (
      Deno.env.get("STAGING_AUTH_URL") ||
      "https://staging--prodai-landing.netlify.app/cli-auth"
    );
  }

  return (
    Deno.env.get("PRODUCTION_AUTH_URL") || "https://pushtoprod.ai/cli-auth"
  );
};
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (error) {
        ...
        if (callbackUrl) {
          const errorURL = `${callbackUrl}?error=${encodeURIComponent(
            error
          )}&error_description=${encodeURIComponent(errorDescription)}`;
          return Response.redirect(errorURL, 302);
        } else {
          const errorURL = `${AUTH_URL}?error=${encodeURIComponent(
            error
          )}&error_description=${encodeURIComponent(errorDescription)}`;
          return Response.redirect(errorURL, 302);
        }
      ...
      if (code && state) {
        try {
          ...
          if (callbackUrl) {
            const successURL = `${callbackUrl}?token=${encodeURIComponent(
              token
            )}`;
            return Response.redirect(successURL, 302);
          } else {
            const successURL = `${AUTH_URL}?success=true&token=${encodeURIComponent(
              token
            )}`;
            return Response.redirect(successURL, 302);
          }
          ...
          if (callbackUrl) {
            const errorURL = `${callbackUrl}?error=token_exchange_failed&error_description=${encodeURIComponent(
              error.message
            )}`;
            return Response.redirect(errorURL, 302);
          } else {
            const errorURL = `${AUTH_URL}?error=token_exchange_failed&error_description=${encodeURIComponent(
              error.message
            )}`;
            return Response.redirect(errorURL, 302);
          }
      ...
      const authPageURL = `${AUTH_URL}?state=${encodeURIComponent(
        state || ""
      )}${
        ...
        if (body.access_token && body.state) {
          try {
            ...
            if (body.callback_url) {
              const successURL = `${
                body.callback_url
              }?token=${encodeURIComponent(cliToken)}`;
              return new Response(
                JSON.stringify({
                  success: true,
                  token: cliToken,
            ...
            if (body.callback_url) {
              const errorURL = `${
                body.callback_url
              }?error=authentication_failed&error_description=${encodeURIComponent(
                error.message
              )}`;
              return new Response(
                JSON.stringify({
