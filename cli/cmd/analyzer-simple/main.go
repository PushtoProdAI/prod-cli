package main

import (
	"encoding/json"
	"fmt"
	"log"
	"os"

	"github.com/meroxa/prod/cli/internal/analyzer"
)

func main() {
	out := os.Stdout
	if len(os.Args) < 2 {
		fmt.Fprintf(out, "Usage: go run cmd/analyzer-simple/main.go <project-path>\n")
		fmt.Fprintf(out, "Example: go run cmd/analyzer-simple/main.go ../test-projects/flask-app\n")
		os.Exit(1)
	}

	projectPath := os.Args[1]

	// Get analyzer for the project
	analyzerPtr, err := analyzer.GetAnalyzer(projectPath)
	if err != nil {
		log.Fatalf("Failed to get analyzer: %v", err)
	}

	// Analyze the project
	spec, err := analyzerPtr.Analyze()
	if err != nil {
		log.Fatalf("Failed to analyze project: %v", err)
	}

	// Display results
	fmt.Fprintf(out, "Project Analysis Results:\n")
	fmt.Fprintf(out, "========================\n")
	fmt.Fprintf(out, "Name: %s\n", spec.Name)
	fmt.Fprintf(out, "Language: %s\n", spec.Language)
	fmt.Fprintf(out, "Services Required: %d\n", len(spec.ServiceRequirements))

	if len(spec.ServiceRequirements) > 0 {
		fmt.Fprintf(out, "\nRequired Services:\n")
		for i, service := range spec.ServiceRequirements {
			fmt.Fprintf(out, "  %d. %s (%s)\n", i+1, service.Type, service.Provider)
		}
	}

	// Output as JSON
	fmt.Fprintf(out, "\nJSON Output:\n")
	jsonData, err := json.MarshalIndent(spec, "", "  ")
	if err != nil {
		log.Fatalf("Failed to marshal JSON: %v", err)
	}
	fmt.Fprintf(out, "%s\n", string(jsonData))
}
